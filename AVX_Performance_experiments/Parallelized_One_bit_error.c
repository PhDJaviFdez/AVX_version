...
#define Square_mtrx boolean NULL;
...
#define MEASUREMENT_LOOPS       ((uint32_t) 30u)  /*!< Number of measurement test loops */
...
#define M                   ((uint32_t) 18u)//128u) /*!< TBD DESCRIPTION */
#define N                   ((uint32_t) 900u)//57600u) /*!< TBD DESCRIPTION */
#define K                   ((uint32_t) 1024u)//576u) /*!< TBD DESCRIPTION */
	...
	char *argv[])
{
	...
	ui32_combinations_a = (uint32_t)strtoul(ar	gv[5], NULL, 10);
...
static uint32_t smm_intel_fletcher_internal(uint32_t ui32_m, uint32_t ui32_n, uint32_t ui32_k, float32_t f32_alpha, const float32_t * const paf32_ma, const float32_t * const paf32_mb, float32_t * const paf32_mc)
{
	...
	uint64_t val_b_lo[2] = { 0u },
		val_b_hi[2] = { 0u },
		val_c_lo[2] = { 0u },
		val_c_hi[2] = { 0u };

	uint32_t val_b_lo_aux[4] = { 0u }, 
		val_b_hi_aux[4] = { 0u }, 
		val_c_lo_aux[4] = { 0u }, 
		val_c_hi_aux[4] = { 0u };
	...
	for (ui32_idx_i = 0; ui32_idx_i < ui32_m; ++ui32_idx_i)
	{
		...
		for (ui32_idx_k = 0; ui32_idx_k < ui32_k; ++ui32_idx_k, ui32_idx_a++)
		{
			...
			for (ui32_idx_j = 0u, ui32_idx_b = ui32_idx_b_ref, ui32_idx_c = ui32_idx_c_ref; ui32_idx_j <= (ui32_n - 8); ui32_idx_j += 8, ui32_idx_b += 8, ui32_idx_c += 8)
			{
				...
				// SIMD instructions, and therefore, it has to be implemented with sequential instructions and it will
				// produce an increment in the performance impact
				memcpy(val_b_lo, &m128i_Fletcher_b_lo, sizeof(float64_t) * 2);
				memcpy(val_b_hi, &m128i_Fletcher_b_hi, sizeof(float64_t) * 2);
				memcpy(val_c_lo, &m128i_Fletcher_c_lo, sizeof(float64_t) * 2);
				memcpy(val_c_hi, &m128i_Fletcher_c_hi, sizeof(float64_t) * 2);
				for (uint32_t ui32_idx_l = 0; ui32_idx_l < 2; ui32_idx_l++)
				{
					Fletcher_aux.ui64 = (uint32_t) * (uint32_t*)&val_b_lo[ui32_idx_l];
					Fletcher_aux.ui32[0] %= 65535;
					Fletcher_aux.ui32[1] %= 65535;
					val_b_lo[ui32_idx_l] = Fletcher_aux.ui64;
					Fletcher_aux.ui64 = (uint32_t) * (uint32_t*)&val_b_hi[ui32_idx_l];
					Fletcher_aux.ui32[0] %= 65535;
					Fletcher_aux.ui32[1] %= 65535;
					val_b_hi[ui32_idx_l] = Fletcher_aux.ui64;
					Fletcher_aux.ui64 = (uint32_t) * (uint32_t*)&val_c_lo[ui32_idx_l];
					Fletcher_aux.ui32[0] %= 65535;
					Fletcher_aux.ui32[1] %= 65535;
					val_c_lo[ui32_idx_l] = Fletcher_aux.ui64;
					Fletcher_aux.ui64 = (uint32_t) * (uint32_t*)&val_c_hi[ui32_idx_l];
					Fletcher_aux.ui32[0] %= 65535;
					Fletcher_aux.ui32[1] %= 65535;
					val_c_hi[ui32_idx_l] = Fletcher_aux.ui64;
				}
				memcpy(&m128i_Fletcher_b_lo, val_b_lo, sizeof(float64_t) * 2);
				memcpy(&m128i_Fletcher_b_hi, val_b_hi, sizeof(float64_t) * 2);
				memcpy(&m128i_Fletcher_c_lo, val_c_lo, sizeof(float64_t) * 2);
				memcpy(&m128i_Fletcher_c_hi, val_c_hi, sizeof(float64_t) * 2);
			}
			// Evaluation of the ES (A value)
			Fletcher_a.ui32 = Fletcher32c_ui32(Fletcher_a, (uint32_t) * (uint32_t*)&f32_a_part);
	...
	memcpy(val_b_lo_aux, &m128i_Fletcher_b_lo, sizeof(float32_t) * 4);
	memcpy(val_b_hi_aux, &m128i_Fletcher_b_hi, sizeof(float32_t) * 4);
	memcpy(val_c_lo_aux, &m128i_Fletcher_c_lo, sizeof(float32_t) * 4);
	memcpy(val_c_hi_aux, &m128i_Fletcher_c_hi, sizeof(float32_t) * 4);
	for (uint32_t ui32_idx_i = 0; ui32_idx_i < 4; ui32_idx_i++)
	{
		Fletcher_b.ui32 = Fletcher32c_ui32(Fletcher_b, val_b_lo_aux[ui32_idx_i]);
		Fletcher_b.ui32 = Fletcher32c_ui32(Fletcher_b, val_b_hi_aux[ui32_idx_i]);
		Fletcher_c.ui32 = Fletcher32c_ui32(Fletcher_c, val_c_lo_aux[ui32_idx_i]);
		Fletcher_c.ui32 = Fletcher32c_ui32(Fletcher_c, val_c_hi_aux[ui32_idx_i]);
	}
...
static uint32_t smm_intel_xor_crc(uint32_t ui32_m, uint32_t ui32_n, uint32_t ui32_k, float32_t f32_alpha, const float32_t * const paf32_ma, const float32_t * const paf32_mb, float32_t * const paf32_mc)
{
	...
	for (ui32_idx_i = 0; ui32_idx_i < ui32_m; ++ui32_idx_i)
	{
		...
		for (ui32_idx_k = 0; ui32_idx_k < ui32_k; ++ui32_idx_k, ui32_idx_a++)
		{
			...
			//				INTERMEDIATE CRC
			// =====================================================
			// Alternative 1) CRC of xor(val_b,val_c) and CRC of A
			// Alternative 2) CRC of val_b and val_c  and CRC of A
			// =====================================================
			// Alternative 1)
			// Evaluation of the ES (B value)
			/*=================
			ui32_xor_a ^= (uint32_t) *((uint32_t*)&f32_a_part);
			ui32_xor = (0 != prev_end) ? (ui32_xor_a ^ ui32_xor_b) ^ ui32_xor_c : ui32_xor_a;
			ui32_crc_a = _mm_crc32_u32(ui32_crc_a, ui32_xor);


			m256i_xor = _mm256_xor_si256(m256i_xor_c, m256i_xor_b);
			uint32_t val[8];
			memcpy(val, &m256i_xor, sizeof(val));
			for (uint32_t ui32_idx_i = 0; ui32_idx_i < 8; ui32_idx_i++)
			{
			...
			ui32_crc_b = _mm_crc32_u32(ui32_crc_b, ui32_xor);
			=======================*/
			// Alternative 2) COMMENTED
			/*ui32_xor_a ^= (uint32_t) *((uint32_t*)&f32_a_part);
			ui32_xor = (0 != prev_end) ? (ui32_xor_a ^ ui32_xor_b) ^ ui32_xor_c : ui32_xor_a;
			ui32_crc_a = _mm_crc32_u32(ui32_crc_a, ui32_xor);
			...
			for (uint32_t ui32_idx_l = 0; ui32_idx_l < 8; ui32_idx_l++)
			{
				//ui32_xor ^= (uint32_t) * (uint32_t*)&val_b[ui32_idx_l];
				ui32_crc_b = _mm_crc32_u32(ui32_crc_b, (uint32_t) * (uint32_t*)&val_b[ui32_idx_l]);
				ui32_crc_c = _mm_crc32_u32(ui32_crc_c, (uint32_t) * (uint32_t*)&val_c[ui32_idx_l]);
			}
			...
			//ui32_xor ^= (uint32_t) *((uint32_t*)&f32_a_part);
			//ui32_crc = _mm_crc32_u32(ui32_crc, ui32_xor);
			//ui32_crc = _mm_crc32_u32(ui32_xor, ui32_crc_b);
			//ui32_crc = _mm_crc32_u32(ui32_crc, ui32_crc_c);
			//ui32_crc = _mm_crc32_u32(ui32_crc, ui32_xor);
			*/
			ui32_idx_b_ref += ui32_n;
